	...
	if(hllMod->symbols != NULL) {
		...
		if(tmp[1] == TYPE_CPPPROC ||
		   tmp[1] == TYPE_CPPMEMFUNC) {
			...
			strncpy(fName, tp->name, tp->cbName);
			fName[tp->cbName] = '\0';
			cppName = demangle(fName, &next, RegularNames);
			if(text(cppName))
			    strcpy(funcName, text(cppName));
			erase(cppName);
			goto findLineOffset;
		    }
		}
		...
		*/
		if(tmp[1] == TYPE_CPPPROC ||
		   tmp[1] == TYPE_CPPMEMFUNC) {
		    ...
		    strncpy(fName, tp->name, tp->cbName);
		    fName[tp->cbName] = '\0';
		    cppName = demangle(fName, &next, 1);
		    if(text(cppName) && (strcpy(text(cppName), funcName) == 0)) {
			debugBuffer->MTE = module->MTE;
			debugBuffer->Value = hllMod->module->segment;
			if(DispatchCommand(DBG_C_NumToAddr)) {
			    erase(cppName);
